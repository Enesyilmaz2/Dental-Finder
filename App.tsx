  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex h-screen overflow-hidden font-sans">
      ...
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} bg-[#0F172A] text-white transition-all duration-300 flex flex-col shrink-0 z-30 shadow-2xl`}>
        <div className={`p-4 h-14 border-b border-white/5 flex items-center ${isSidebarOpen ? 'justify-between' : 'justify-center'}`}>
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-4 scrollbar-hide">
          <button onClick={() => { setPageMode('HOME'); setSelectedCityFolder(null); }} className={`w-full flex items-center gap-4 p-3 rounded-2xl transition-all ${pageMode === 'HOME' && !selectedCityFolder ? 'bg-blue-600 shadow-lg shadow-blue-600/20' : 'hover:bg-white/5 text-slate-500'}`}>
            <i className="fas fa-home w-5 text-center text-base"></i>
            ...
            {cityGroups.map(([city, count]) => (
              <button key={city} onClick={() => { setPageMode('CITY_LISTS'); setSelectedCityFolder(city); }} className={`w-full flex items-center justify-between p-2.5 rounded-xl transition-all ${selectedCityFolder === city ? 'bg-white/10 text-white' : 'hover:bg-white/5 text-slate-500'}`}>
                <div className="flex items-center gap-3">
                  ...
                  {isSidebarOpen && <span className="text-[11px] font-black uppercase tracking-tight truncate">{city}</span>}
                </div>
                {isSidebarOpen && <span className="text-[9px] font-black bg-blue-600/20 text-blue-400 px-2 py-0.5 rounded-lg">{count}</span>}
              </button>
            ))}
          </div>
          ...
          <div className="space-y-1">
            <div className={`px-3 mb-1 flex items-center justify-between ${!isSidebarOpen && 'justify-center'}`}>
              <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">{isSidebarOpen ? 'CRM' : 'CRM'}</span>
            </div>
            <button onClick={() => setPageMode('CONVERSATIONS_POSITIVE')} className={`w-full flex items-center gap-3 p-2.5 rounded-xl transition-all ${pageMode === 'CONVERSATIONS_POSITIVE' ? 'bg-emerald-500 text-white shadow-md' : 'hover:bg-white/5 text-slate-500'}`}>
              <i className="fas fa-smile w-5 text-center text-base"></i>
            ...
            <button onClick={() => setPageMode('CONVERSATIONS_NEGATIVE')} className={`w-full flex items-center gap-3 p-2.5 rounded-xl transition-all ${pageMode === 'CONVERSATIONS_NEGATIVE' ? 'bg-red-500 text-white shadow-md' : 'hover:bg-white/5 text-slate-500'}`}>
              <i className="fas fa-frown w-5 text-center text-base"></i>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white">
        ...
        <header className="h-12 bg-white border-b border-slate-100 flex items-center px-4 gap-4 shrink-0 z-20">
          {!isSidebarOpen && (
            <span className="font-macondo font-black text-blue-600 tracking-wider text-xl shrink-0">dentalMap</span>
          )}
          ...
          <div className="flex-1 max-w-xl relative">
            ...
            <input 
              type="text" 
              placeholder="Şehir ve ilçe (Örn: Malatya Yeşilyurt)..." 
              className="w-full bg-slate-50 border border-slate-200 pl-9 pr-3 py-1 rounded-xl text-[11px] outline-none focus:border-blue-500 transition-all font-bold text-slate-800"
              value={citySearch}
          ...
          <button 
            ...
            disabled={isLoading} 
            className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-1.5 rounded-xl text-[10px] font-black shadow-md shadow-blue-500/10 disabled:opacity-50 transition-all shrink-0 uppercase tracking-widest"
          >
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-6 bg-slate-50/30">
          {error && (
            <div className="mb-4 p-3 bg-red-50 border border-red-100 text-red-600 rounded-xl flex items-center gap-3">
            ...
            <div className="mb-6 bg-white p-4 rounded-2xl border border-blue-100 shadow-xl shadow-blue-500/5 animate-in slide-in-from-top-4">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                   <div className="flex justify-between items-end mb-1">
                     <h4 className="text-blue-900 font-black text-[11px] uppercase tracking-widest">Arama devam ediyor...</h4>
              ...
              <div className="mt-3 h-1 w-full bg-slate-100 rounded-full overflow-hidden">
                <div 
                  ...
                  style={{ width: `${Math.min((processedCount/targetCount) * 100, 100)}%` }}
                ></div>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 gap-4">
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <p className="text-slate-400 text-[9px] font-black uppercase tracking-[0.3em]">{filteredClinics.length} kayıt bulundu</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => exportToExcel(filteredClinics)} className="bg-emerald-600 text-white px-4 py-1.5 rounded-xl text-[9px] font-black hover:bg-emerald-700 transition-all flex items-center gap-2 shadow-md shadow-emerald-500/10 uppercase tracking-widest">
                <i className="fas fa-file-excel text-xs"></i> EXCEL
              </button>
            </div>
          </div>
          ...
          {filteredClinics.length === 0 && !isLoading && (
            <div className="flex flex-col items-center justify-center py-32 text-slate-200">
              ...
              <p className="font-black text-sm tracking-tight text-slate-300 uppercase">Arama sonucu bulunamadı</p>
            </div>
          )}
        </main>

        {/* Footer - Daraltıldı & Yazı Boyutu Düzenlendi */}
          ...
          <div className="flex flex-col gap-2 max-w-screen-2xl mx-auto">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-bold text-slate-600">Toplam: {stats.total}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div>
                <span className="text-[9px] font-bold text-emerald-600">Olumlu: {stats.positive}</span>
              </div>
              <div className="flex items-center gap-1.5">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-bold text-red-600">Olumsuz: {stats.negative}</span>
              </div>
            </div>

            {/* Copyright Area */}
            <div className="flex flex-col items-center justify-center text-center pt-1.5 border-t border-slate-50">
